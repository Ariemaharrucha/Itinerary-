...
export const LocationForm = () => {
  ...
  function handleSelectPopuler(value: string) {
    setValue(value);
    setStepData('location', value);
    nextStep();
  }
  ...
  return (
    <section className="w-2/3 m-auto">
      ...
      <div className="mt-16">
        <h1 className="text-center text-3xl text-md">
          Atau memulai dengan tujuan populer
        </h1>
        <div className="mt-8 flex justify-around">
          {populerLocation.map((location) => (
            <div key={location.value} className="relative text-center cursor-pointer group" onClick={()=>handleSelectPopuler(location.value)}>
              <div className="overflow-hidden rounded-md size-32 relative">
                <img
                  src={location.img}
                  alt={location.label}
                  className="w-full h-full object-cover"
                />
              <div className="absolute inset-0 bg-white opacity-0 group-hover:opacity-25 transition-opacity"></div>
              </div>
